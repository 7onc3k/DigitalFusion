...
class FileInsertApp:
    def __init__(self):
        ...
        self.selected_files = set()  # Nový atribut pro ukládání označených souborů
    ...
    def open_file_selector(self):
        ...
        self.file_listbox = tk.Listbox(self.selector, height=15, width=50, selectmode=tk.MULTIPLE)
        ...
        self.selector.bind('<space>', self.toggle_selection)
    ...
    def update_file_list(self):
        ...
        for file in self.files:
            ...
            if file in self.selected_files:
                self.file_listbox.itemconfig(tk.END, {'bg': 'lightblue'})
    ...
    def filter_files(self, event):
        ...
        for root, dirs, files in os.walk(self.directory):
            for file in files:
                full_path = os.path.join(root, file)
                rel_path = os.path.relpath(full_path, self.directory)
                if search_term in rel_path.lower() or rel_path in self.selected_files:
                    self.files.append(rel_path)
        self.update_file_list()
    ...
    def toggle_selection(self, event):
        if self.files:
            ...
            if selected_file in self.selected_files:
                ...
                self.file_listbox.itemconfig(self.current_selection, {'bg': 'white'})
            else:
                self.selected_files.add(selected_file)
                self.file_listbox.itemconfig(self.current_selection, {'bg': 'lightblue'})
        return 'break'

    def select_file(self, event):
        if not self.selected_files:
            if self.files:
                self.selected_files.add(self.files[self.current_selection])
        ...
        if self.selected_files:
            ...
            pyperclip.copy(full_content.strip())
            ...
            print(f"Obsah souborů vložen: {len(full_content)} znaků")
